defmodule ZaimuTomo.DocumentProcessing.Saga do
  @moduledoc """
  Process manager that reacts to `:document_uploaded` events
  and starts the OCR workflow via `ZaimuTomo.DocumentProcessing.OCRSupervisor`.
  ...
  @impl true
  def init(state) do
    Phoenix.PubSub.subscribe(ZaimuTomo.PubSub, "documents_uploaded")
    {:ok, state}
  end
  ...
  @impl true
  def handle_info({:created, %ZaimuTomo.Documents.Document{} = document}, state) do
    case ZaimuTomo.DocumentProcessing.process_document(document) do
      {:ok, _pid} ->
