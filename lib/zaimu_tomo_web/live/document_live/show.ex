  ...
  def render(assigns) do
    ...
    <div class="grid grid-12">
      <div class="card span-7">
        ...
        <%= if data do %>
          <div class="budget-row" style="border-bottom:1px solid var(--hairline);padding-bottom:10px;margin-bottom:10px">
          ...
          <div class="budget-row"><div class="name muted">Issuer</div><div>{data.issuer || "—"}</div></div>
          <div class="budget-row"><div class="name muted">Date</div><div>{data.invoice_date || "—"}</div></div>
          <div class="budget-row"><div class="name muted">Invoice #</div><div class="mono dim">{data.invoice_number || "—"}</div></div>
          <div class="budget-row"><div class="name muted">Currency</div><div>{data.currency || "—"}</div></div>
          <div class="budget-row"><div class="name muted">Reason</div><div>{data.reason_for_payment || "—"}</div></div>
        <% else %>
      ...
      <div class="card span-5">
        ...
        <div class="budget-row"><div class="name muted">Filename</div><div class="mono dim">{@document.filename}</div></div>
        <div class="budget-row"><div class="name muted">Uploaded</div><div>{ZaimuTomoWeb.Layouts.rel_time(DateTime.to_iso8601(@document.inserted_at))}</div></div>
